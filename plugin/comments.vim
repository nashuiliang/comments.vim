...
function! CommentLine()
  ...
  " for .cpp or .hpp or .java or .C or .go files use //
  if file_name =~ '\.cpp$' || file_name =~ '\.hpp$' || file_name =~ '\.java$' || file_name =~ '\.php[2345]\?$' || file_name =~ '\.C$' || file_name =~ '\.go$' || file_name =~ '\.rs$'
  ...
  " for .vim files use "
  elseif file_name =~ '\.vim$' || file_name =~ '\.vimrc$' || file_name =~ 'vimrc$'
	 execute ":silent! normal ^i\"\<ESC>\<down>^"
  " for .sql files use --
